...
def main(
    ...
    source_column: str = typer.Option(
        ..., "--source_column", help="Column in the right file to copy into the new column"
    ),
    ...
    filter_column: str = typer.Option(
        None, "--filter_column", help="Keep only rows where filter_column == filter_value (optional)"
    ),
    filter_value: str = typer.Option(
        None, "--filter_value", help="Keep only rows where ffilter_column == filter_value (optional)"
    ),
    ...
    # Merge dataframes
    merged_df = left_df.merge(right_min, how="left", left_on=left_id, right_on=right_id)
