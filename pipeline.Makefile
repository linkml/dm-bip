...
# Repo manifest with pre-captured git metadata (generated by Dockerfile)
DM_REPO_MANIFEST ?=

DEBUG_TARGETS := pipeline-debug map-debug schema-debug validate-debug help

# Output directory ownership guard
# ============
# An output directory belongs to exactly one DM_SCHEMA_NAME. provenance.yaml
# already records the owner, so compare it before anything writes here.
#
# The relationship is one-directional: a directory has exactly one schema name,
# but a schema name legitimately has many directories (one per consent group, one
# per timestamped run — see bdc-workflow.sh). Only the first direction is checked.
# Re-running the same study into the same directory is the normal incremental
# case and stays silent, whatever changed about the specs or the entity subset.
#
# This has to run at parse time rather than in a recipe. $(PREPARED_INPUT_MK) is
# pulled in with -include just below, and make remakes included makefiles before
...
# asserts that it actually fires.
_GUARD_EXEMPT_GOALS := $(DEBUG_TARGETS) clean $(filter %-clean,$(MAKECMDGOALS))
_REAL_WORK := $(if $(strip $(MAKECMDGOALS)),$(strip $(filter-out $(_GUARD_EXEMPT_GOALS),$(MAKECMDGOALS))),default)

# Read unconditionally (it is guarded by $(wildcard), so a fresh directory costs
# no subprocess). output-clean needs it too, to remove the *previous* owner's
# schema file rather than one named after whatever DM_SCHEMA_NAME is set now.
ifneq ($(wildcard $(PROVENANCE_FILE)),)
_PRIOR_SCHEMA_NAME := $(shell awk '/^pipeline:/{f=1;next} /^[^[:space:]]/{f=0} f && /^  schema_name:/{sub(/^  schema_name:[ ]*/, ""); gsub(/^["'"'"']|["'"'"']$$/, ""); print; exit}' $(PROVENANCE_FILE) 2>/dev/null)
endif

ifndef DM_ALLOW_OUTPUT_REUSE
ifneq ($(_REAL_WORK),)
ifneq ($(_PRIOR_SCHEMA_NAME),)
ifneq ($(_PRIOR_SCHEMA_NAME),$(DM_SCHEMA_NAME))
$(error DM_OUTPUT_DIR ($(DM_OUTPUT_DIR)) holds products for '$(_PRIOR_SCHEMA_NAME)', but DM_SCHEMA_NAME is '$(DM_SCHEMA_NAME)'. Point DM_OUTPUT_DIR somewhere else, run 'make output-clean' to reset this directory, or set DM_ALLOW_OUTPUT_REUSE=1 to write into it anyway)
endif
endif
endif
endif
