...
PROVENANCE_FILE := $(DM_OUTPUT_DIR)/provenance.yaml

# External git repos for provenance tracking (space-separated paths)
DM_EXTERNAL_REPOS ?=
...
$(PROVENANCE_FILE):
	...
	$(RUN) python -m dm_bip.provenance \
		...
		$(if $(DM_EXTERNAL_REPOS),$(foreach repo,$(DM_EXTERNAL_REPOS),--repo $(repo)),--no-external-repos)
...
$(SCHEMA_FILE): $(INPUT_FILES) | $(PROVENANCE_FILE) $(if $(DM_RAW_SOURCE),$(PREPARED_INPUT_MK))
