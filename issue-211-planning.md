...
`Demography.sex`, `Demography.race`, `Observation.value_enum` are plain strings.
...
Key features:
- `populated_from` — one-to-one mapping (invertible)
- `sources` — many-to-one (not invertible)
- `mirror_source: true` — unmapped values pass through unchanged
- `mirror_source: false` (default) — unmapped values are dropped

## Proposed plan

### 1. Add enums to the target schema

`target-schema.yaml` currently has no enums. For enum derivations to be meaningful,
the target schema probably needs enum definitions for fields like `sex`, `race`,
`ethnicity`, `condition_status`, `value_enum`, etc.

**Question for team:** Should we add target enums, or is the intent to derive enums
that don't yet exist in the target schema? (i.e., is the target schema intentionally
enum-free?)
