#!/usr/bin/env python3
"""Parse a --trans-spec slug into newline-separated fields.

Slug format: OWNER/REPO[@REF][:PATH]

Prints four lines on stdout, in this order:
    OWNER_REPO
    REPO_NAME
    REF              (empty line if absent)
    EXPLICIT_PATH    (empty line if absent)
...
def main(slug: str = typer.Argument(..., help="Slug of the form OWNER/REPO[@REF][:PATH]")):
    """Parse the slug and print fields one per line."""
    try:
    ...
    print(fields["owner_repo"])
    print(fields["repo_name"])
    print(fields["ref"])
    print(fields["explicit_path"])
